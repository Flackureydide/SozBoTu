...
def help(update, context):
    update.message.reply_text('Mövcud əmrlər:\n' +
                              '/@Crolog Söz Oyun u ilə bağlı hansısa problem olsa @CrologDestey Qurupuna gələrəy dəstəy ala bilərsiz\n' +
...
def command_start(update, context: CallbackContext):
    if update.effective_chat.type == "private":
        ...
        addme = InlineKeyboardButton(text="➕ Məni Öz Qurupuna Əlavə Et ➕", url="https://t.me/neutralsozbot?startgroup=a")
        oyun = InlineKeyboardButton(text="🤖 Yeniliklər 🤖", url="https://t.me/Kamodu")
        admin = InlineKeyboardButton(text="Owner 👨‍💻", url="https://t.me/Miko_45")

        keyboard = [[addme],[sohbet],[oyun],[admin]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        update.message.reply_text('Salam 👋 Mən sizin qurupda Söz Oyunu oynamağınız üçün yaradılmış Botam mənimlə birlikdə söz bazanızı dahada artıra və inkişaf elətdirə bilərsiz', reply_to_message_id=True, reply_markup=reply_markup)
    else:
        ...
        update.message.reply_text('Neutarl Söz Oyunu Başladı ⚡'.format(username), reply_to_message_id=True)
...
def set_master(update, context):
    ...
    show_word_btn = InlineKeyboardButton("▶️ Sözə bax", callback_data='show_word')
